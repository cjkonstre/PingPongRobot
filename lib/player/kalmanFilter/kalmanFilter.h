...
class KalmanModel {
    ...
    Eigen::Matrix<double, 6, 3> get_kalmanGain() const; //returns simplified kalman gain (K'), dependent on currect covariant matrix

public:
    KalmanModel(const Eigen::Matrix<double, 3, 3>& sensor_noise, const double& noise_sigma, const double& dt); //3d etimation noise, process noise
    ...
    void set_dt(const double& dt); //sets all relevant matricies as well 

    Eigen::Matrix<double, 6, 1> predict_state() const; // w/ dt
    Eigen::Matrix<double, 6, 6> predict_covariance() const; // w/ dt

    void update(const Eigen::Matrix<double, 3, 1>& pos_measurement); //updates internal state w/ measurement, w/ dt
};